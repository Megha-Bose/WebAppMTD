...
def getAttackStackelberg(def_util, strat, P, stack_soln, rng):
	y = rng.random()
	tau = NUMTYPES -1
	...
	for dataset_num in range(int(sys.argv[1]), int(sys.argv[2]) + 1):
		...
		for iter in range(MAX_ITER):
			...
			for t in range(T):
				...
				for i in range(NUMSTRATS):
					util[i], typ[i], attack[i] = getAttackStackelberg(def_util, strat[i], Pvec, stack_soln, rng)
		...
		# print("\n")
		stdout = sys.stdout
		f_ov_out = open(OUT_DIR + str(dataset_num) + "overall_out_stackelberg.txt", 'w')
		sys.stdout = f_ov_out
		...
		f_out = open(OUT_DIR + str(dataset_num) + "output_stackelberg.txt", "w")
		for i in range(NUMSTRATS):
