...
def getAttackRandom(def_util, att_util, strat, P, rng):
	y = rng.random()
	tau = NUMTYPES -1
	...
	for a in range(NUMATTACKS):
		if(att_util[tau, a] > 0):
			strats.append(a)
	v = int(strats[int(rng.random()*len(strats))])
	...
	for dataset_num in range(int(sys.argv[1]), int(sys.argv[2]) + 1):
		...
		for iter in range(MAX_ITER):
			...
			for t in range(T):
				...
				for i in range(NUMSTRATS):
					util[i], typ[i], attack[i] = getAttackRandom(def_util, att_util, strat[i], Pvec, rng)
		...
		# print("\n")
		stdout = sys.stdout
		f_ov_out = open(OUT_DIR + str(dataset_num) + "overall_out_random.txt", 'w')
		sys.stdout = f_ov_out
		...
		f_out = open(OUT_DIR + str(dataset_num) + "output_random.txt", "w")
		for i in range(NUMSTRATS):
